...
```json
{
  "scripts": {
    ...
    // 单独构建 web-antd 应用
    "build:antd": "pnpm run build --filter=@olinc/web-antd",
    // 单独构建文档
    "build:docs": "pnpm run build --filter=@olinc/docs",
    // 单独构建 web 应用
    "build:ele": "pnpm run build --filter=@olinc/eb-ele",
    // 单独构建 web-naive 应用
    "build:naive": "pnpm run build --filter=@olinc/aive",
    // 单独构建 web-tdesign 应用
    "build:tdesign": "pnpm run build --filter=@olinc/eb-tdesign",
    // 单独构建 playground 应用
    "build:play": "pnpm run build --filter=@olinc/layground",
    // changeset 版本管理
    ...
    "dev": "turbo-run dev",
    // 启动web-antd应用
    "dev:antd": "pnpm -F @olinc/eb-antd run dev",
    // 启动文档
    "dev:docs": "pnpm -F @olinc/ocs run dev",
    // 启动web-ele应用
    "dev:ele": "pnpm -F @olinc/eb-ele run dev",
    // 启动web-naive应用
    "dev:naive": "pnpm -F @olinc/eb-naive run dev",
    // 启动演示应用
    "dev:play": "pnpm -F @olinc/layground run dev",
    // 格式化代码
...
以`@olinc/eb-antd`添加测试环境`test`为例：
...
"scripts": {
    "build": "cross-env NODE_OPTIONS=--max-old-space-size=8192 turbo build",
    "build:analyze": "turbo build:analyze",
    "build:antd": "pnpm run build --filter=@olinc/eb-antd",
    "build-test:antd": "pnpm run build --filter=@olinc/eb-antd build:test",
...
"tasks": {
    ...
    "build-test:antd": {
      "dependsOn": ["@olinc/eb-antd#build:test"],
      "outputs": ["dist/**"]
    },

    "@olinc/eb-antd#build:test": {
      "dependsOn": ["^build"],
      "outputs": ["dist/**"]
    },
