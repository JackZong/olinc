...
静态路由和动态路由都会走权限控制，可以通过配置路由的 `meta` 属性中的 `authority` 字段来控制权限，可以参考[路由权限控制](https://github.com/JackZong/olinc/blob/main/playground/src/router/routes/modules/demos.ts)。
...
import { OLINC_LOGO_URL } from '@olinc/constants';
...
const routes: RouteRecordRaw[] = [
  {
    meta: {
      ...
      icon: OLINC_LOGO_URL,
      order: 9999,
      title: $t('page.olinc.title'),
    },
    name: 'OlincProject',
    path: '/olinc-adminn',
    redirect: '/olinc-adminn/about',
    children: [
      {
        name: 'OlincAbout',
...
import { useRefresh } from '@olinc/hooks';
