...
```bash [.env]
# 应用标题
VITE_APP_TITLE=Olinc Admin

# 应用命名空间，用于缓存、store等功能的前缀，确保隔离
VITE_APP_NAMESPACE=olinc-web-antd
```
...
# 接口地址
VITE_GLOB_API_URL=https://mock-napi.olinc.pro/api
...
```ts
window._OLINC_ADMIN_PRO_APP_CONF_ = {
  VITE_GLOB_API_URL: 'https://mock-napi.olinc.pro/api',
};
Object.freeze(window._OLINC_ADMIN_PRO_APP_CONF_);
Object.defineProperty(window, '_OLINC_ADMIN_PRO_APP_CONF_', {
  configurable: false,
  writable: false,
});
```
...
想要获取 `_app.config.js` 内的变量，需要使用`@olinc/hooks`提供的 `useAppConfig`方法。
...
- 首先在 `.env` 或者对应的开发环境配置文件内，新增需要可动态配置的变量，需要以 `VITE_GLOB_*` 开头的变量，如：
  ...
  ```bash
  VITE_GLOB_OTHER_API_URL=https://mock-napi.olinc.pro/other-api
  ```

- 在 `packages/types/global.d.ts`,新增对应的类型定义，如：

  ```ts
  export interface OlincAdminProAppConfigRaw {
...
- 在 `packages/effects/hooks/src/use-app-config.ts` 中，新增对应的配置项，如：
  ...
  export function useAppConfig(
    ...
    // 生产环境下，直接使用 window._OLINC_ADMIN_PRO_APP_CONF_ 全局变量
    const config = isProduction
      ? window._OLINC_ADMIN_PRO_APP_CONF_
      : (env as OlincAdminProAppConfigRaw);
...
import { defineOverridesPreferences } from '@olinc/preferences';
...
const defaultPreferences: Preferences = {
  app: {
    ...
    contentPaddingTop: 0,
    defaultAvatar:
      'https://unpkg.com/@olincjs/static-source@0.1.7/source/avatar-v1.webp',
  ...
  copyright: {
    companyName: 'Olinc',
    companySiteLink: 'https://www.olinc.pro',
    date: '2024',
    enable: true,
    icp: '',
    icpLink: '',
  ...
  logo: {
    ...
    fit: 'contain',
    source:
      'https://unpkg.com/@olincjs/static-source@0.1.7/source/logo-v1.webp',
    // sourceDark: 'https://unpkg.com/@olincjs/static-source@0.1.7/source/logo-dark.webp', // 可选：暗色主题logo
  },
  navigation: {
    accordion: true,
    split: true,
