...
项目中的样式文件存放在 `@olinc/styles`，包含一些全局样式，如重置样式、全局变量等，它继承了 `@olinc-core/design` 的样式和能力，可以根据项目需求进行覆盖。
...
import { useNamespace } from '@olinc/hooks';
