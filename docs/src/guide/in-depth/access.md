...
有了权限码，就可以使用 `@olinc/ccess` 提供的`AccessControl`组件及API来进行按钮的显示与隐藏。

#### 组件方式

```vue
<script lang="ts" setup>
import { AccessControl, useAccess } from '@olinc/ccess';
...
```vue
<script lang="ts" setup>
import { AccessControl } from '@olinc/ccess';
</script>
...
```vue
<script lang="ts" setup>
import { useAccess } from '@olinc/ccess';
