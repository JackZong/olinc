...
import type { Recordable } from '@olinc-core/typings';

import type { FormActions, FormSchema, OlincFormProps } from './types';

import { isRef, toRaw } from 'vue';

import { Store } from '@olinc-core/shared/store';
import {
  bindMethods,
  createMerge,
  formatDate,
  ...
  StateHandler,
} from '@olinc-core/shared/utils';

function getDefaultState(): OlincFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<OlincFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | OlincFormProps = null;
  stateHandler: StateHandler;

  public store: Store<OlincFormProps>;
  ...
  private prevState: null | OlincFormProps = null;

  constructor(options: OlincFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<OlincFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: OlincFormProps) => Partial<OlincFormProps>)
      | Partial<OlincFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<OlincForm /> is not mounted');
    }
    return this.form;
  }
