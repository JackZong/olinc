...
import { useSimpleLocale } from '@olinc-core/composables';
import {
  CircleAlert,
  CircleCheckBig,
  CircleHelp,
      ...
      <div :class="cn('relative flex-1 overflow-y-auto p-3', contentClass)">
        ...
        <AlertDialogDescription>
          <div class="m-4 min-h-[30px]">
            <OlincRenderContent :content="content" render-br />
          </div>
          <OlincLoading v-if="loading && contentMasking" :spinning="loading" />
        </AlertDialogDescription>
        <div
          class="flex items-center justify-end gap-x-2"
          :class="`justify-${buttonAlign}`"
        >
          <OlincRenderContent :content="footer" />
          <AlertDialogCancel v-if="showCancel" as-child>
            <component
