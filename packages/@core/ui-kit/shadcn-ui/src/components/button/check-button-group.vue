...
import type { OlincButtonGroupProps, ValueType } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@olinc-core/icons';
import { cn, isFunction } from '@olinc-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { OlincRenderContent } from '../render-content';
import OlincButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<OlincButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
...
<template>
  <OlincButtonGroup
    :size="props.size"
    :gap="props.gap"
    class="olinc-check-button-group"
  >
    <Button
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <OlincRenderContent :content="btn.label" />
      </slot>
    </Button>
  </OlincButtonGroup>
</template>
<style lang="scss" scoped>
.olinc-check-button-group {
  display: flex;
  flex-wrap: wrap;
