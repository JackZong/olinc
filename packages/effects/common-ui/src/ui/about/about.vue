...
import { OlincRenderContent } from '@olinc-core/shadcn-ui';
...
declare global {
  const __OLINC_ADMIN_METADATA__: {
    authorEmail: string;
    authorName: string;
    authorUrl: string;
    buildTime: string;
...
const renderLink = (href: string, text: string) =>
  h(
    'a',
    { href, target: '_blank', class: 'olinc-link' },
    { default: () => text },
  );
...
const vbenDescriptionItems: DescriptionItem[] = [
  ...
  {
    content: renderLink(OLINC_DOC_URL, '点击查看'),
    title: '文档地址',
  },
  {
    content: renderLink(OLINC_PREVIEW_URL, '点击查看'),
    title: '预览地址',
  },
  {
    content: renderLink(OLINC_GITHUB_URL, '点击查看'),
    title: 'Github',
  },
  {
    content: h('div', [
  ...
  <Page :title="title">
    <template #description>
      <p class="text-foreground mt-3 text-sm leading-6">
        <a :href="OLINC_GITHUB_URL" class="olinc-link" target="_blank">
          {{ name }}
        </a>
        {{ description }}
      </p>
          ...
          <template v-for="item in vbenDescriptionItems" :key="item.title">
            <div class="border-border border-t px-4 py-6 sm:col-span-1 sm:px-0">
              ...
              <dd class="text-foreground mt-1 text-sm leading-6 sm:mt-2">
                <OlincRenderContent :content="item.content" />
              </dd>
            </div>
          </template>
        </dl>
