...
import type { OlincFormProps } from '@olinc-core/form-ui';
...
import { OlincHelpTooltip, OlincLoading } from '@olinc-core/shadcn-ui';

import { usePriorityValues } from '@olinc/hooks';
import { EmptyIcon } from '@olinc/icons';
import { $t } from '@olinc/locales';
import { usePreferences } from '@olinc/preferences';
import {
  cloneDeep,
  cn,
  isBoolean,
...
async function init() {
  ...
  // form 由 olinc-form代替，所以不适配formConfig，这里给出警告
  const formConfig = gridOptions.value?.formConfig;
  // 处理某个页面加载多个Table时，第2个之后的Table初始化报出警告
  // 因为第一次初始化之后会把defaultGridOptions和gridOptions合并后缓存进State
  if (formConfig && formConfig.enabled) {
    console.warn(
      '[Olinc Vxe Table]: The formConfig in the grid is not supported, please use the `formOptions` props',
    );
  }
  props.api?.setState?.({ gridOptions: defaultGridOptions });
  // form 由 olinc-form 代替，所以需要保证query相关事件可以拿到参数
...
watch(
  ...
  () => {
    formApi.setState((prev) => {
      const finalFormOptions: OlincFormProps = mergeWithArrayOverride(
        {},
        formOptions.value,
        prev,
      );
      ...
      <template v-if="showToolbar" #toolbar-actions="slotProps">
        <slot v-if="showTableTitle" name="table-title">
          <div class="mr-1 pl-1 text-[1rem]">
            {{ tableTitle }}
            <OlincHelpTooltip v-if="tableTitleHelp" trigger-class="pb-1">
              {{ tableTitleHelp }}
            </OlincHelpTooltip>
          </div>
        </slot>
