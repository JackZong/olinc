...
import type { DrawerPlacement, DrawerState } from '@olinc/common-ui';

import { Page, useOlincDrawer } from '@olinc/common-ui';
...
const [BaseDrawer, baseDrawerApi] = useOlincDrawer({
  // 连接抽离的组件
  connectedComponent: BaseDemo,
  // placement: 'left',
});

const [InContentDrawer, inContentDrawerApi] = useOlincDrawer({
  // 连接抽离的组件
  connectedComponent: inContentDemo,
  // placement: 'left',
});

const [AutoHeightDrawer, autoHeightDrawerApi] = useOlincDrawer({
  connectedComponent: AutoHeightDemo,
});

const [DynamicDrawer, dynamicDrawerApi] = useOlincDrawer({
  connectedComponent: DynamicDemo,
});

const [SharedDataDrawer, sharedDrawerApi] = useOlincDrawer({
  connectedComponent: SharedDataDemo,
});

const [FormDrawer, formDrawerApi] = useOlincDrawer({
...
<template>
  ...
  >
    <template #extra>
      <DocButton path="/components/common-ui/olinc-drawer" />
    </template>
    <BaseDrawer />
    <InContentDrawer />
    <AutoHeightDrawer />
