...
const [BaseModal, baseModalApi] = useOlincModal({
  // 连接抽离的组件
  connectedComponent: BaseDemo,
});

const [InContentModal, inContentModalApi] = useOlincModal({
  // 连接抽离的组件
  connectedComponent: InContentModalDemo,
});

const [AutoHeightModal, autoHeightModalApi] = useOlincModal({
  connectedComponent: AutoHeightDemo,
});

const [DragModal, dragModalApi] = useOlincModal({
  connectedComponent: DragDemo,
});

const [DynamicModal, dynamicModalApi] = useOlincModal({
  connectedComponent: DynamicDemo,
});

const [SharedDataModal, sharedModalApi] = useOlincModal({
  connectedComponent: SharedDataDemo,
});

const [FormModal, formModalApi] = useOlincModal({
  connectedComponent: FormModalDemo,
});

const [NestedModal, nestedModalApi] = useOlincModal({
  connectedComponent: NestedDemo,
});

const [BlurModal, blurModalApi] = useOlincModal({
...
<template>
  ...
  >
    <template #extra>
      <DocButton path="/components/common-ui/olinc-modal" />
    </template>
    <BaseModal />
    <InContentModal />
    <AutoHeightModal />
    ...
    <Flex wrap="wrap" class="w-full" gap="10">
      ...
      <Card class="w-[300px]" title="轻量提示弹窗">
        <template #extra>
          <DocButton path="/components/common-ui/olinc-alert" />
        </template>
